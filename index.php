...
<body class="font-sans text-sm text-black">
    <div class="shell mx-auto mt-5 min-h-[680px] border border-gray-500">
        ...
        <main class="px-7 py-5">
            ...
            <form id="order-form" action="controller.php?action=create" method="post" class="mx-auto max-w-md">
                <label class="mb-4 flex items-center"><span class="w-28">Nama</span><input required name="nama" class="h-8 flex-1 border border-gray-400 px-2"></label>
                <label class="mb-4 flex items-center"><span class="w-28">Nomor HP</span><input required name="no_telepon" type="tel" class="h-8 flex-1 border border-gray-400 px-2"></label>
                <div class="mb-5 flex items-start">
                    ...
                    <div class="relative flex-1">
                        ...
                        <div id="room-menu" class="absolute z-10 hidden max-h-48 w-full overflow-y-auto border border-gray-400 bg-white p-2 shadow peer-checked:block">
                            <div id="room-options">
                                <?php foreach ($available_rooms as $nomor => $kamar): ?>
                                    <label class="block cursor-pointer px-1 py-1 hover:bg-blue-50"><input type="checkbox" name="rooms[]" value="<?= htmlspecialchars($nomor) ?>" class="mr-2">Room <?= htmlspecialchars($nomor) ?></label>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    </div>
